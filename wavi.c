...
int main() {
    ...
    uint32_t data_size = SAMPLE_RATE * 2 * 2; // 2 segundos * 2 canales
    uint32_t i;
    ...
    header.num_channels = 2; // 2 canales (estéreo)
    header.sample_rate = SAMPLE_RATE;
    header.bits_per_sample = 16;
    header.byte_rate = header.sample_rate * header.num_channels * header.bits_per_sample / 8;
    header.block_align = header.num_channels * header.bits_per_sample / 8;
    header.subchunk2_id = 0x61746164; // "data"
    header.subchunk2_size = data_size * header.bits_per_sample / 8;
    header.chunk_size = 36 + header.subchunk2_size;

    // Aloca el buffer de audio
    data_buffer = (uint16_t*)malloc(data_size * sizeof(uint16_t));

    // Genera una onda sinusoidal de 440 Hz durante 2 segundos para el canal izquierdo
    for (i = 0; i < data_size; i += 2) {
        double t = (double)i / SAMPLE_RATE;
        data_buffer[i] = (uint16_t)(AMPLITUDE * sin(2 * M_PI * 440 * t));
    }

    // Genera una onda sinusoidal de 880 Hz durante 2 segundos para el canal derecho
    for (i = 1; i < data_size; i += 2) {
        double t = (double)i / SAMPLE_RATE;
        data_buffer[i] = (uint16_t)(AMPLITUDE * sin(2 * M_PI * 880 * t));
    }
